...
class weighted_MSELoss(nn.Module):
    ...
    def forward(self, targets, inputs, weights):
        res = (inputs - targets)**2
        res = res.mean([1, 2, 3])
        return res * weights
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        out = [z, cond]
        if return_first_stage_outputs:   # validation runs here
            xrec = self.decode_first_stage(z)
            out.extend([x, xrec])
        if return_original_cond:  # validation runs here
            out.append(xc)

        if 'reward' in batch.keys():
            reward = super().get_input(batch, 'reward')
            out.append(reward)
        return out
    ...
    def shared_step(self, batch, **kwargs):
        if 'reward' not in batch.keys():
            x, c = self.get_input(batch, self.first_stage_key)
            loss = self(x, c)
        else:
            x, c, reward = self.get_input(batch, self.first_stage_key)
            loss = self(x, c, reward)
        return loss

    def forward(self, x, c, reward=None, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        ...
        return self.p_losses(x, c, t, reward, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, reward=None, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        if reward is None:
            ...
            loss_simple = self.weighted_mse_loss(target, model_output, reward)
